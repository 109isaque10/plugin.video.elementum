...
def run(url_suffix="", retry=0):
    ...
    #Plays with jacktorr instead of using elementum, for caching and privacy purposes. Can use any other addon too.
    if '/play' in url:
        uri = sys.argv[2].split("uri=")[-1]
        if uri.endswith('.torrent'):
            url = "plugin://plugin.video.jacktorr/play_url?url="+uri
            xbmc.executebuiltin(f'PlayMedia({url})')
            return
        else:
            url = "plugin://plugin.video.jacktorr/play_magnet?magnet="+uri
            xbmc.executebuiltin(f'PlayMedia({url})')
            return
    query_add = ""
